...
class QueryInfoAtLocation{
  ...
  final  metaService = MetadataService();
    ...
    Future< List<StoringProduct>? > listWithIrLocation(String locationCode) async {
      final result = await metaService.queryInfo(locationCode, includesIrCode : true);
            ...
            if (it.serials.isEmpty) {
                result.add(
                    InboundProduct(
                        ...
                        conditionTypeId : it.conditionTypeId.toInt(),
...
return data!.items!.where((it) => it.productType == ProductType.cargo).toList();
    }
